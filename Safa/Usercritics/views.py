    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                return redirect(f'Usercritics:read:{c.id}')
            else:
                return render (request, 'create.html', {'form': form})
        else:
            return render (request, 'create.html', {'form': CriticForm()})
    else:
        return redirect('Usercritics:login')
...
def deletec(request, id):
    ...
    if request.user.id == c.creator.id:
        c.delete()
        return redirect('Usercritics:create')

def signup(request):
    if request.user.is_authenticated:
        return redirect ('Usercritics:logout')
    else:
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                login(request, user)
                return redirect('Usercritics:list')
            else:
                return render(request, 'create.html', {'form':form})
        else:
